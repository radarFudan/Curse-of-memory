...
class CustomLinearLayer(nn.Module):
    ...
    def stability_margin(self):
        ...
        if self.training:
            ...
            print(self.weight[positive_weights_mask].shape)
...
class KOAKDiagonalRNN(nn.Module):
    def __init__(
        self,
        rec1_size: int = 128,
        ...
        self.U = nn.Linear(rec1_size, rec1_size, dtype=torch.float64, bias=True)
        self.alpha = 0.01
        self.r = 2
    ...
    def forward(self, x):
        ...
        for i in range(input_length):
            h_tilde = torch.tanh(self.W(hidden[i]) + self.U(x[:, i : i + 1, :]))
            h_next = hidden[i] + self.alpha * (
                h_tilde - hidden[i] * torch.pow(torch.abs(hidden[i]), self.r)
            )
...
if __name__ == "__main__":
    ...
    # SimpleDiagonalRNN
    train_model = KOAKDiagonalRNN(d, training=True)
    test_model = KOAKDiagonalRNN(d, training=False)
