...
# https://raw.githubusercontent.com/HazyResearch/state-spaces/main/models/s4/s4.py
# For variation unify, rename rec1_size into rec1_size
...
def LinearActivation(
    ...
    **kwargs,
    ...
    linear_cls = partial(nn.Conv1d, kernel_size=1, dtype=dtype) if transposed else nn.Linear
    if activation is not None and activation == "glu":
        d_output *= 2
    linear = linear_cls(d_input, d_output, bias=bias, dtype=dtype, **kwargs)
...
def rank_correction(measure, N, rank=1, dtype=torch.float64):
...
def nplr(
    measure, N, rank=1, dtype=torch.float64, diagonalize_precision=True, B_clip=2.0, training=True
):
...
class Kernel(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        rec1_size (H): Model dimension, or number of independent convolution kernels created.
        ...
        super().__init__()
        assert rec1_size > 0
        self.H = self.rec1_size = rec1_size
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        state: Initial state for recurrent updates.
            E.g. for SSMs, this should have shape (B, H, N) (batch, rec1_size, d_state).
        rate: Relative sampling rate.
        L: Target kernel length.

        Returns:
          - (C, H, L) (channels, rec1_size, l_kernel) The convolution kernel.
          - (B, H, L) (batch, rec1_size, l_kernel)
    ...
    def forward_state(self, u, state):
        """Forward the state through a sequence, i.e. computes the state after passing chunk
        through the kernel."""
        raise NotImplementedError
...
class SSMKernel(Kernel):
    ...
    Options:
    d_state (N): State size (dimensionality of parameters A, B, C). Generally shouldn't need to be adjusted and doesn't affect speed much for most kernels (e.g. S4, S4D).
    ...
    def __init__(
        ...
        **init_args,
        ...
        self.N = d_state
        # self.dtype, self.cdtype = torch.float64, torch.cfloat
        self.dtype, self.cdtype = torch.float64, torch.cdouble
...
class SSMKernelDiag(SSMKernel):
    ...
    def register_params(self, A, B, C, inv_dt, P):
        ...
        H (or rec1_size): total SSM copies
...
class SSMKernelDPLR(SSMKernelDiag):
    """SSM kernel for diagonal + low rank (DPLR) state matrices, corresponding to the original S4
    model."""
    ...
    def _setup_C(self, L):
        ...
        Two modes are supported: go directly to length L if self.l_kernel is 1, or length is
        doubled
        """
    ...
    def _omega(self, L, dtype, device, cache=True):
        ...
        This also caches a version of the nodes "unprocessed" with the bilinear transform. This
        method should be called every time the internal length self.l_kernel changes.
        """
    ...
    def forward(self, state=None, rate=1.0, L=None):
        ...
        if state is not None:
            ...
            sA = s * _conj(A) - contract("bhm, rhm, rhn -> bhn", s, _conj(Q), _conj(P))  # (B H N)
            s = s / dt + sA / 2
            s = s[..., : self.N]
        ...
        v = v * dt  # Incorporate dt into B, dt is (H N)
    ...
    def _step_state_linear(self, u=None, state=None):
        """Version of the step function that has time O(N) instead of O(N^2) per step, which takes
        advantage of the DPLR form and bilinear discretization.
    ...
    def _step_state(self, u, state):
        ...
        next_state = torch.einsum(self.state_contraction, self.dA, state) + torch.einsum(
    ...
    def step(self, u, state):
        """Must have called self._setup_step() and created state with self.default_state() before
        calling this."""
    ...
    def stability_margin(self):
        ...
        smallest_positive = real_eigvals.min()
...
class FFTConv(nn.Module):
    ...
    rec1_size (H): Model dimension (in CNN terminology, this would be "channels").
    l_max (L): The maximum kernel length. Set l_max=None to always use a global kernel.
    channels: Can be interpreted as a number of "heads"; the SSM is a map from a 1-dim to C-dim sequence. It's not recommended to change this; instead, increase rec1_size for larger models.
    ...
    def __init__(
        ...
        **kernel_args,  # Arguments passed into inner convolution kernel
        ...
        self.D = nn.Parameter(torch.randn(channels, self.rec1_size, dtype=torch.float64))
        ...
        print("In s4.py, kernel is S4 is", kernel)
        self.kernel = kernel_cls(
            rec1_size=self.rec1_size,
            l_max=self.l_max,
            channels=channels,
            **kernel_args,
        )
    ...
    @property
    def d_output(self):
        return self.rec1_size * self.channels

    def stability_margin(self):
        """Return the stability margin of the model."""

        return self.kernel.stability_margin()

    def perturb_weight_initialization(self):
        """Perturb the weight initialization to make the model unstable."""

        self.kernel.perturb_weight_initialization()


class S4Block(nn.Module):
    """General block design wrapping an inner layer. Currently only layer=FFTConv is supported, but
    easy to incorporate others.
    ...
    def __init__(
        ...
        **layer_args,  # Arguments into inner layer (e.g. FFTConv)
        ...
        if bottleneck is not None:
            self.rec1_size = self.rec1_size // bottleneck
        ...
        if gate is not None:
            ...
            if self.layer.d_output != self.rec1_size * gate:
                self.output_gate = LinearActivation(
                    self.rec1_size * self.channels,
                    self.rec1_size * gate,
                    transposed=False,
                    activation=None,
                    activate=False,
    ...
    def forward(self, x, lengths=None, **kwargs):  # absorbs return_output and transformer src mask
        ...
        if lengths is not None:
            ...
            mask = torch.where(
                torch.arange(L, device=lengths.device)[:, None] < lengths[:, None, None],
                1.0,
                0.0,
            )
            x = x * mask
        ...
        # return y, state  # Modified, output only
        # return y  # Modified, output only
    ...
    @property
    def d_output(self):
        return self.rec1_size

    def stability_margin(self):
        """Return the stability margin of the model."""

        return self.layer.stability_margin()

    def perturb_weight_initialization(self):
        """Perturb the weight initialization to make the model unstable."""

        self.layer.perturb_weight_initialization()


if __name__ == "__main__":
    d = 10
    # the first two are input dimension and output dimension
    test_model = S4Block(d, transposed=False)

    inputs = torch.randn(1, 100, d)
    ...
    # step check
    # test_model.step(inputs[:, 0, :], test_model.default_state(1))
